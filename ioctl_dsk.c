...
typedef struct { UCHAR Infotype; UCHAR DriveUnit; } DSKREQ;

BOOL drvReady(UCHAR devno) {
  BIOSPARAMETERBLOCK bpb = {0}; DSKREQ DskReq;
  ULONG cbParam = sizeof(DskReq), cbData = sizeof(bpb);
  DskReq.Infotype = 0; DskReq.DriveUnit = devno;
  return DosDevIOCtl( -1, IOCTL_DISK, DSK_GETDEVICEPARAMS, (PVOID)&DskReq, sizeof(DskReq), &cbParam,
                            (PVOID)&bpb, sizeof(bpb), &cbData ) == NO_ERROR;
}
